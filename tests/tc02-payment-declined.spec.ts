...
test.describe('TC-02: Payment declined', () => {
  test('should keep user on payment page after decline and allow retry with valid card', async ({ page }) => {
    ...
    // Keep page focused on the declined message before retry
    const declineMessage = page.getByText(/your card has been declined/i).first();
    await expect(declineMessage).toBeVisible();
    await page.evaluate(() => window.scrollTo({ top: 0, behavior: 'instant' }));
    await declineMessage.scrollIntoViewIfNeeded();
    await page.waitForTimeout(2500);
    console.log('❌ Declined card message is visible before retry');
